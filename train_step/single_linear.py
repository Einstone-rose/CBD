...
def build_ft_model(args, configs, device, checkpoint=None):
    ...
    # model_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(model_optimizer, T_max=configs.finetune_epoch, eta_min=configs.ft_lr/10)
...
def Trainer(train_dl, val_dl, test_dl, device, logger, args, configs, experiment_log_dir, start_epoch, load_path):
            ...
            if ft_epoch % 5 == 0:
                ...
                if best_performance is None:
                    ...
                    if performance[0] > best_performance[0]:
                        ...
                        should_save = True
                        logger.debug(
                            'EP%s - Better Testing: Acc=%.4f| Precision = %.4f | Recall = %.4f | F1 = %.4f ｜ valLoss = %.4f | valAcc = %.4f' % (
                                                            ft_epoch, performance[0], performance[1], performance[2], performance[3], valid_loss, val_acc))
    ...
    logger.debug('EP%s - Best Testing: Acc=%.4f| Precision = %.4f | Recall = %.4f | F1 = %.4f' % (
                ft_epoch, best_performance[0], best_performance[1], best_performance[2], best_performance[3]))
    logger.debug("=" * 100)
